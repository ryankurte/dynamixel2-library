...
{
    return DXL_build_pkt(id, DX_INSTR_PING, 0, NULL, max_len, length, packet);
}

int DXL_parse_ping_resp(uint8_t length, uint8_t* packet, uint8_t *id, uint8_t* error,
                                uint16_t *model, uint8_t* firmware)
{
    ...
    res = DXL_parse_status_pkt(length, packet, id, error,
                               ...
                               uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    return DXL_build_pkt(id, DX_INSTR_READ, sizeof(data), data, max_len, length, packet);
}
...
{
    return DXL_parse_status_pkt(length, packet, id, error,
                                ...
                                uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    return DXL_build_pkt(id, DX_INSTR_WRITE, data_count + 2, packed_data, max_len, length, packet);
}
                                   ...
                                   uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    return DXL_build_pkt(id, DX_INSTR_REG_WRITE, data_count + 2, packed_data, max_len, length, packet);
}
...
{
    return DXL_build_pkt(id, DX_INSTR_ACTION, 0, NULL, max_len, length, packet);
}

int DXL_build_factory_rst_req(uint8_t id, uint8_t reset_mode, uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    return DXL_build_pkt(id, DX_INSTR_FACTORY_RESET, 1, &reset_mode, max_len, length, packet);
}

int DXL_build_reboot_req(uint8_t id, uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    return DXL_build_pkt(id, DX_INSTR_REBOOT, 0, NULL, max_len, length, packet);
}
                                   ...
                                   uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    return DXL_build_pkt(id, DX_INSTR_SYNC_READ, sizeof(data), data, max_len, length, packet);
}
                                    ...
                                    uint8_t* write_data, uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    return DXL_build_pkt(id, DX_INSTR_SYNC_WRITE, sizeof(data), data, max_len, length, packet);
}
                          ...
                          uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    *length = HEADER_SIZE + packet_length;
    ...
    // Compute CRC
    uint16_t crc = DXL_compute_crc(HEADER_SIZE + param_count + 1, packet);
                                ...
                                uint8_t max_params, uint8_t *param_count, uint8_t *params)
{
    ...
    uint16_t crc_check = DXL_compute_crc(length - CRC_SIZE, packet);
