...
uint16_t Manager::ComputeCRC(uint16_t length, uint8_t *data)
{
    uint16_t i, j;
...
int Manager::BuildPing(uint8_t id)
{
                        ...
                        uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    return Manager::BuildPacket(id, DX_INSTR_WRITE, data_count + 2, packed_data, max_len, length, packet);
}
                         ...
                         uint8_t max_len, uint8_t* length, uint8_t* packet)
{
    ...
    *length = HEADER_SIZE + packet_length;
    ...
    // Compute CRC
    uint16_t crc = Manager::ComputeCRC(HEADER_SIZE + param_count + 1, packet);
                      ...
                      uint8_t max_params, uint8_t *param_count, uint8_t *params)
{
    ...
    // Check CRC
    uint16_t crc = Manager::ComputeCRC(length, packet);
